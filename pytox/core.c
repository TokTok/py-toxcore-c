...
static void callback_log(Tox *tox, TOX_LOG_LEVEL level, const char *file, uint32_t line,
                         const char *func, const char *message, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_log", "isiss", level, file, line, func, message);
}

static void callback_self_connection_status(Tox *tox, TOX_CONNECTION connection_status,
                                            void *self) {
  PyObject_CallMethod((PyObject *)self, "on_self_connection_status", "i", connection_status);
}

static void callback_friend_request(Tox *tox, const uint8_t *public_key, const uint8_t *data,
...
static void callback_friend_message(Tox *tox, uint32_t friendnumber, TOX_MESSAGE_TYPE type,
  ...
  PyObject_CallMethod((PyObject *)self, "on_friend_message", "iis#", friendnumber, type, message,
                      length - (message[length - 1] == 0));
}

static void callback_friend_name(Tox *tox, uint32_t friendnumber, const uint8_t *newname,
                                 size_t length, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_friend_name", "is#", friendnumber, newname,
                      length - (newname[length - 1] == 0));
}

static void callback_friend_status_message(Tox *tox, uint32_t friendnumber,
                                           const uint8_t *newstatus, size_t length, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_friend_status_message", "is#", friendnumber, newstatus,
                      length - (newstatus[length - 1] == 0));
}

static void callback_friend_status(Tox *tox, uint32_t friendnumber, TOX_USER_STATUS status,
                                   void *self) {
  PyObject_CallMethod((PyObject *)self, "on_friend_status", "ii", friendnumber, status);
}

static void callback_friend_typing(Tox *tox, uint32_t friendnumber, bool is_typing, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_friend_typing", "iO", friendnumber,
                      PyBool_FromLong(is_typing));
}

static void callback_friend_read_receipt(Tox *tox, uint32_t friendnumber, uint32_t receipt,
                                         void *self) {
  PyObject_CallMethod((PyObject *)self, "on_friend_read_receipt", "ii", friendnumber, receipt);
}

static void callback_friend_connection_status(Tox *tox, uint32_t friendnumber,
                                              TOX_CONNECTION status, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_friend_connection_status", "iO", friendnumber,
                      PyBool_FromLong(status));
}
...
static void callback_conference_invite(Tox *tox, uint32_t friendnumber, TOX_CONFERENCE_TYPE type,
  ...
  PyObject_CallMethod((PyObject *)self, "on_conference_invite", "ii" BUF_TC "#", friendnumber, type,
                      data, length);
}

static void callback_conference_message(Tox *tox, uint32_t conference_number, uint32_t peer_number,
...
static void callback_conference_peer_name(Tox *tox, uint32_t conference_number,
                                          uint32_t peer_number, const uint8_t *name, size_t length,
                                          void *self) {
  PyObject_CallMethod((PyObject *)self, "on_conference_peer_name", "ii" BUF_TC "#",
                      conference_number, peer_number, name, length);
}

static void callback_conference_peer_list_changed(Tox *tox, uint32_t conference_number,
                                                  void *self) {
...
static void callback_file_chunk_request(Tox *tox, uint32_t friend_number, uint32_t file_number,
  ...
  PyObject_CallMethod((PyObject *)self, "on_file_chunk_request", "iiKi", friend_number, file_number,
                      position, length);
}

static void callback_file_recv(Tox *tox, uint32_t friend_number, uint32_t file_number,
  ...
  if (kind == TOX_FILE_KIND_AVATAR && filename != NULL) {
    ...
    PyObject_CallMethod((PyObject *)self, "on_file_recv", "iiiKs#", friend_number, file_number,
                        kind, file_size, filename_hex, TOX_HASH_LENGTH * 2);
  } else {
    PyObject_CallMethod((PyObject *)self, "on_file_recv", "iiiKs#", friend_number, file_number,
...
static void callback_file_recv_control(Tox *tox, uint32_t friend_number, uint32_t file_number,
  ...
  PyObject_CallMethod((PyObject *)self, "on_file_recv_control", "iii", friend_number, file_number,
                      control);
}
...
static void callback_file_recv_chunk(Tox *tox, uint32_t friend_number, uint32_t file_number,
                                     ...
                                     void *self) {
  PyObject_CallMethod((PyObject *)self, "on_file_recv_chunk", "iiK" BUF_TC "#", friend_number,
                      file_number, position, data, length);
}
...
static PyObject *ToxCore_friend_add(ToxCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s#s#", &address, &addr_length, &data, &data_length)) {
    return NULL;
  }
  ...
  switch (err) {
    ...
    case TOX_ERR_FRIEND_ADD_ALREADY_SENT:
      PyErr_SetString(ToxOpError,
                      "friend request already sent or already "
                      "a friend");
      break;
    ...
    case TOX_ERR_FRIEND_ADD_SET_NEW_NOSPAM:
      PyErr_SetString(ToxOpError,
                      "the friend was already there but the "
...
static PyObject *ToxCore_friend_get_connection_status(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_self_set_status_message(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_friend_get_status_message_size(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_friend_get_status_message(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_self_get_status_message(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_self_get_status_message_size(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_self_get_friend_list_size(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_conference_peer_get_name(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
  ...
  tox_conference_peer_get_name(self->tox, conference_number, peer_number, buf, &error);
...
static PyObject *ToxCore_conference_join(ToxCore *self, PyObject *args) {
  ...
  uint32_t ret = tox_conference_join(self->tox, friend_number, cookie, length, &error);
...
static PyObject *ToxCore_conference_send_message(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_conference_peer_number_is_ours(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_conference_get_chatlist_size(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_conference_get_chatlist(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyObject *ToxCore_file_send(ToxCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iiKss#", &friend_number, &kind, &file_size, &file_id, &filename,
                        &filename_length)) {
    return NULL;
  }

  TOX_ERR_FILE_SEND err = 0;
  uint32_t file_number = tox_file_send(self->tox, friend_number, kind, file_size, file_id, filename,
                                       filename_length, &err);
...
static PyObject *ToxCore_file_send_chunk(ToxCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iiKs#", &friend_number, &file_number, &position, &data, &length)) {
    return NULL;
  }

  TOX_ERR_FILE_SEND_CHUNK err = 0;
  bool ret =
      tox_file_send_chunk(self->tox, friend_number, file_number, position, data, length, &err);
  if (!ret) {
...
static PyObject *ToxCore_self_get_keys(ToxCore *self, PyObject *args) {
  ...
  PyTuple_SetItem(res, 0,
                  PYSTRING_FromStringAndSize((char *)public_key_hex, TOX_PUBLIC_KEY_SIZE * 2));
  PyTuple_SetItem(res, 1,
                  PYSTRING_FromStringAndSize((char *)secret_key_hex, TOX_PUBLIC_KEY_SIZE * 2));
  return res;
}
...
static PyObject *ToxCore_bootstrap(ToxCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s#Hs#", &address, &addr_length, &port, &public_key, &pk_length)) {
    return NULL;
  }
...
static PyObject *ToxCore_self_get_connection_status(ToxCore *self, PyObject *args) {
  CHECK_TOX(self);
...
static PyMethodDef Tox_methods[] = {
    ...
    {
        "friend_get_connection_status", (PyCFunction)ToxCore_friend_get_connection_status,
        METH_VARARGS,
    ...
    {
        "self_get_status_message", (PyCFunction)ToxCore_self_get_status_message, METH_NOARGS,
        "self_get_status_message()\n"
        "Get status message of yourself.",
    },
    {
        "self_get_status_message_size", (PyCFunction)ToxCore_self_get_status_message_size,
        METH_NOARGS,
    ...
    {
        "self_get_friend_list_size", (PyCFunction)ToxCore_self_get_friend_list_size, METH_NOARGS,
        "self_get_friend_list_size()\n"
        "Return the number of friends.",
    },
    {
        "self_get_friend_list", (PyCFunction)ToxCore_self_get_friend_list, METH_NOARGS,
    ...
    {
        "conference_get_chatlist_size", (PyCFunction)ToxCore_conference_get_chatlist_size,
        METH_VARARGS,
        "conference_get_chatlist_size()\n"
        "Return the number of conferences in the current Tox instance.",
    },
    {
        "conference_peer_number_is_ours", (PyCFunction)ToxCore_conference_peer_number_is_ours,
        METH_VARARGS,
