...
cdef void handle_self_connection_status(Tox* tox, Tox_Connection connection_status, void* user_data) noexcept:
    (<Core> user_data).handle_self_connection_status(Tox_Connection(connection_status))
cdef void handle_friend_name(Tox *tox,
    ...
    (<Core> user_data).handle_friend_name(friend_number, name, length)
cdef void handle_friend_status_message(
    ...
    (<Core> user_data).handle_friend_status_message(friend_number, message, length)
cdef void handle_friend_status(
    Tox* tox, Tox_Friend_Number friend_number, Tox_User_Status status, void* user_data) noexcept:
    (<Core> user_data).handle_friend_status(friend_number, Tox_User_Status(status))
cdef void handle_friend_connection_status(
    ...
    (<Core> user_data).handle_friend_connection_status(friend_number, Tox_Connection(connection_status))
cdef void handle_friend_typing(
    Tox* tox, Tox_Friend_Number friend_number, bool typing, void* user_data) noexcept:
    (<Core> user_data).handle_friend_typing(friend_number, typing)
cdef void handle_friend_read_receipt(
    ...
    (<Core> user_data).handle_friend_read_receipt(friend_number, message_id)
cdef void handle_friend_request(
    Tox* tox, const uint8_t public_key[],
    const uint8_t message[], size_t length,
    void* user_data) noexcept:
    (<Core> user_data).handle_friend_request(public_key, message, length)
cdef void handle_friend_message(
    Tox* tox, Tox_Friend_Number friend_number, Tox_Message_Type type,
    const uint8_t message[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_friend_message(friend_number, Tox_Message_Type(type), message, length)
cdef void handle_file_recv_control(
    ...
    void* user_data) noexcept:
    (<Core> user_data).handle_file_recv_control(friend_number, file_number, Tox_File_Control(control))
cdef void handle_file_chunk_request(
    ...
    (<Core> user_data).handle_file_chunk_request(friend_number, file_number, position, length)
cdef void handle_file_recv(
    ...
    (<Core> user_data).handle_file_recv(friend_number, file_number, kind, file_size, filename, filename_length)
cdef void handle_file_recv_chunk(
    Tox* tox, Tox_Friend_Number friend_number, Tox_File_Number file_number, uint64_t position,
    const uint8_t data[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_file_recv_chunk(friend_number, file_number, position, data, length)
cdef void handle_conference_invite(
    ...
    (<Core> user_data).handle_conference_invite(friend_number, Tox_Conference_Type(type), cookie, length)
cdef void handle_conference_connected(Tox* tox, Tox_Conference_Number conference_number, void* user_data) noexcept:
    (<Core> user_data).handle_conference_connected(conference_number)
cdef void handle_conference_message(
    ...
    (<Core> user_data).handle_conference_message(conference_number, peer_number, Tox_Message_Type(type), message, length)
cdef void handle_conference_title(
    Tox* tox, Tox_Conference_Number conference_number, Tox_Conference_Peer_Number peer_number,
    const uint8_t title[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_conference_title(conference_number, peer_number, title, length)
cdef void handle_conference_peer_name(
    Tox* tox, Tox_Conference_Number conference_number, Tox_Conference_Peer_Number peer_number,
    const uint8_t name[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_conference_peer_name(conference_number, peer_number, name, length)
cdef void handle_conference_peer_list_changed(Tox* tox, Tox_Conference_Number conference_number, void* user_data) noexcept:
    (<Core> user_data).handle_conference_peer_list_changed(conference_number)
cdef void handle_friend_lossy_packet(
    Tox* tox, Tox_Friend_Number friend_number,
    const uint8_t data[], size_t length,
    void* user_data) noexcept:
    (<Core> user_data).handle_friend_lossy_packet(friend_number, data, length)
cdef void handle_friend_lossless_packet(
    Tox* tox, Tox_Friend_Number friend_number,
    const uint8_t data[], size_t length,
    void* user_data) noexcept:
    (<Core> user_data).handle_friend_lossless_packet(friend_number, data, length)
cdef void handle_group_peer_name(
    Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id,
    const uint8_t name[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_group_peer_name(group_number, peer_id, name, length)
cdef void handle_group_peer_status(Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id, Tox_User_Status status,
    void* user_data) noexcept:
    (<Core> user_data).handle_group_peer_status(group_number, peer_id, Tox_User_Status(status))
cdef void handle_group_topic(
    Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id,
    const uint8_t topic[], size_t length,
    void* user_data) noexcept:
    (<Core> user_data).handle_group_topic(group_number, peer_id, topic, length)
cdef void handle_group_privacy_state(Tox* tox, Tox_Group_Number group_number, Tox_Group_Privacy_State privacy_state,
    void* user_data) noexcept:
    (<Core> user_data).handle_group_privacy_state(group_number, Tox_Group_Privacy_State(privacy_state))
cdef void handle_group_voice_state(Tox* tox, Tox_Group_Number group_number, Tox_Group_Voice_State voice_state,
    void* user_data) noexcept:
    (<Core> user_data).handle_group_voice_state(group_number, Tox_Group_Voice_State(voice_state))
...
cdef void handle_group_message(
    ...
    (<Core> user_data).handle_group_message(group_number, peer_id, Tox_Message_Type(type), message, length, message_id)
cdef void handle_group_private_message(
    Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id, Tox_Message_Type type,
    const uint8_t message[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_group_private_message(group_number, peer_id, Tox_Message_Type(type), message, length)
cdef void handle_group_custom_packet(
    Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id,
    const uint8_t data[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_group_custom_packet(group_number, peer_id, data, length)
cdef void handle_group_custom_private_packet(
    Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id,
    const uint8_t data[], size_t length, void* user_data) noexcept:
    (<Core> user_data).handle_group_custom_private_packet(group_number, peer_id, data, length)
cdef void handle_group_invite(
    Tox* tox, Tox_Friend_Number friend_number,
    const uint8_t invite_data[], size_t length,
    const uint8_t group_name[], size_t group_name_length,
    void* user_data) noexcept:
    (<Core> user_data).handle_group_invite(friend_number, invite_data, length, group_name, group_name_length)
cdef void handle_group_peer_join(Tox* tox, Tox_Group_Number group_number, Tox_Group_Peer_Number peer_id, void* user_data) noexcept:
    (<Core> user_data).handle_group_peer_join(group_number, peer_id)
cdef void handle_group_peer_exit(
    ...
    (<Core> user_data).handle_group_peer_exit(group_number, peer_id, Tox_Group_Exit_Type(exit_type), name, name_length, part_message, part_message_length)
cdef void handle_group_self_join(Tox* tox, Tox_Group_Number group_number, void* user_data) noexcept:
    (<Core> user_data).handle_group_self_join(group_number)
cdef void handle_group_join_fail(Tox* tox, Tox_Group_Number group_number, Tox_Group_Join_Fail fail_type, void* user_data) noexcept:
    (<Core> user_data).handle_group_join_fail(group_number, Tox_Group_Join_Fail(fail_type))
cdef void handle_group_moderation(
    ...
    (<Core> user_data).handle_group_moderation(group_number, source_peer_id, target_peer_id, Tox_Group_Mod_Event(mod_type))
