...
from pytox.toxcore.tox cimport Tox
cdef extern from "tox/toxav.h":
    ...
    ctypedef void toxav_call_cb(Toxav* av, uint32_t friend_number, bool audio_enabled, bool video_enabled, void* user_data)
    ctypedef void toxav_call_state_cb(Toxav* av, uint32_t friend_number, uint32_t state, void* user_data)
    ctypedef void toxav_audio_bit_rate_cb(Toxav* av, uint32_t friend_number, uint32_t audio_bit_rate, void* user_data)
    ctypedef void toxav_audio_receive_frame_cb(Toxav* av, uint32_t friend_number, const int16_t* pcm, size_t sample_count, uint8_t channels, uint32_t sampling_rate, void* user_data)
    ctypedef void toxav_video_bit_rate_cb(Toxav* av, uint32_t friend_number, uint32_t video_bit_rate, void* user_data)
    ctypedef void toxav_video_receive_frame_cb(Toxav* av, uint32_t friend_number, uint16_t width, uint16_t height, const uint8_t* y, const uint8_t* u, const uint8_t* v, int32_t ystride, int32_t ustride, int32_t vstride, void* user_data)
