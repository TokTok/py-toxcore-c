...
static void ToxAVCore_callback_call(ToxAV *toxAV, uint32_t friend_number,
                                    bool audio_enabled, bool video_enabled,
                                    void *self) {
  PyObject_CallMethod((PyObject *)self, "on_call", "iii", friend_number,
                      audio_enabled, video_enabled);
}

static void ToxAVCore_callback_call_state(ToxAV *toxAV, uint32_t friend_number,
                                          uint32_t state, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_call_state", "ii", friend_number,
                      state);
}

static void ToxAVCore_callback_audio_bit_rate(ToxAV *toxAV,
...
static void ToxAVCore_callback_video_bit_rate(ToxAV *toxAV,
                                              uint32_t friend_number,
...
static void ToxAVCore_callback_audio_receive_frame(
    ToxAV *toxAV, uint32_t friend_number, const int16_t *pcm,
    size_t sample_count, uint8_t channels, uint32_t sampling_rate, void *self) {
  PyGILState_STATE gstate = PyGILState_Ensure();

  uint32_t length = sample_count * channels * 2;
  PyObject_CallMethod((PyObject *)self, "on_audio_receive_frame",
                      "i" BUF_TCS "iii", friend_number, (const char *)pcm,
                      length, sample_count, channels, sampling_rate);
...
static void i420_to_rgb(int width, int height, const uint8_t *y,
                        const uint8_t *u, const uint8_t *v, int ystride,
                        int ustride, int vstride, unsigned char *out) {
  const int w = width;
  const int w2 = w / 2;
  ...
  for (line = 0; line < img->d_h; ++line) {
    if (!(line % 2)) {
      for (x = 0; x < img->d_w; x += 2) {
        ...
        img->planes[VPX_PLANE_U][upos++] =
            ((-38 * r + -74 * g + 112 * b) >> 8) + 128;
        img->planes[VPX_PLANE_V][vpos++] =
...
static void ToxAVCore_callback_video_receive_frame(
    ToxAV *toxAV, uint32_t friend_number, uint16_t width, uint16_t height,
  ...
  i420_to_rgb(width, height, y, u, v, ystride, ustride, vstride,
              self->out_image);
  ...
  PyObject_CallMethod((PyObject *)self, "on_video_receive_frame", "iii" BUF_TCS,
                      friend_number, self->o_w, self->o_h, self->out_image,
                      buf_size);
 ...
 */
static void
ToxAVCore_callback_add_av_groupchat(/*Tox*/ void *tox, uint32_t groupnumber,
  ...
  PyObject_CallMethod((PyObject *)self, "on_add_av_groupchat",
                      "ii" BUF_TCS "iii", groupnumber, peernumber, pcm, length,
...
static void
ToxAVCore_callback_join_av_groupchat(/*Tox*/ void *tox, uint32_t groupnumber,
                                     uint32_t peernumber, const int16_t *pcm,
                                     unsigned int samples, uint8_t channels,
                                     unsigned int sample_rate, void *self) {
  PyGILState_STATE gstate = PyGILState_Ensure();
  ...
  PyObject_CallMethod((PyObject *)self, "on_join_av_groupchat",
                      "ii" BUF_TCS "iii", groupnumber, peernumber, pcm, length,
                      samples, channels, sample_rate);
...
static int init_helper(ToxAVCore *self, PyObject *args) {
  ...
  toxav_callback_audio_bit_rate(self->av, ToxAVCore_callback_audio_bit_rate,
                                self);
  toxav_callback_video_bit_rate(self->av, ToxAVCore_callback_video_bit_rate,
                                self);
  toxav_callback_audio_receive_frame(
      self->av, ToxAVCore_callback_audio_receive_frame, self);
  toxav_callback_video_receive_frame(
      self->av, ToxAVCore_callback_video_receive_frame, self);
...
static PyObject *ToxAVCore_new(PyTypeObject *type, PyObject *args,
                               PyObject *kwds) {
...
static PyObject *ToxAVCore_call(ToxAVCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "iii", &friend_number, &audio_bit_rate,
                        &video_bit_rate)) {
    return NULL;
  }
  ...
  bool ret =
      toxav_call(self->av, friend_number, audio_bit_rate, video_bit_rate, &err);
  if (ret == false) {
...
static PyObject *ToxAVCore_call_control(ToxAVCore *self, PyObject *args) {
  uint32_t friend_number;
  uint32_t control;
...
static PyObject *ToxAVCore_audio_set_bit_rate(ToxAVCore *self, PyObject *args) {
  uint32_t friend_number;
  ...
  bool ret =
      toxav_audio_set_bit_rate(self->av, friend_number, audio_bit_rate, &err);
  if (ret == false) {
...
static PyObject *ToxAVCore_video_set_bit_rate(ToxAVCore *self, PyObject *args) {
  uint32_t friend_number;
  ...
  Toxav_Err_Bit_Rate_Set err = 0;
  bool ret =
      toxav_video_set_bit_rate(self->av, friend_number, video_bit_rate, &err);
  if (ret == false) {
    PyErr_Format(ToxOpError, "toxav bit rate set error: %d", err);
    return NULL;
  }
...
static PyObject *ToxAVCore_audio_send_frame(ToxAVCore *self, PyObject *args) {
  uint32_t friend_number;
  int16_t *pcm = NULL;
  uint32_t pcm_length;
  ...
  if (!PyArg_ParseTuple(args, "i" BUF_TCS "iii", &friend_number,
                        (uint8_t **)&pcm, &pcm_length, &sample_count, &channels,
                        &sampling_rate)) {
    return NULL;
  }

  Toxav_Err_Send_Frame err = 0;
  bool ret = toxav_audio_send_frame(self->av, friend_number, pcm, sample_count,
                                    channels, sampling_rate, &err);
  if (ret == false) {
...
static PyObject *ToxAVCore_video_send_frame(ToxAVCore *self, PyObject *args) {

  uint32_t friend_number = 0, len = 0, width = 0, height = 0;
  char *data = NULL;

  if (!PyArg_ParseTuple(args, "iii" BUF_TCS, &friend_number, &width, &height,
                        &data, &len)) {
    return NULL;
  }
  ...
  Toxav_Err_Send_Frame err = 0;
  bool ret = toxav_video_send_frame(
      self->av, friend_number, width, height, self->in_image->planes[0],
...
static PyObject *ToxAVCore_answer(ToxAVCore *self, PyObject *args) {
  uint32_t friend_number;
  uint32_t audio_bit_rate;
  uint32_t video_bit_rate;
  ...
  bool ret = toxav_answer(self->av, friend_number, audio_bit_rate,
                          video_bit_rate, &err);
  if (ret == false) {
...
static PyObject *ToxAVCore_join_av_groupchat(ToxAVCore *self, PyObject *args) {
  uint32_t friend_number;
  uint8_t *data;
  uint32_t length;
  ...
  Tox *tox = ((ToxCore *)self->core)->tox;
  bool ret =
      toxav_join_av_groupchat(tox, friend_number, data, length,
...
static PyObject *ToxAVCore_group_send_audio(ToxAVCore *self, PyObject *args) {
  ...
  Tox *tox = ((ToxCore *)self->core)->tox;
  int ret = toxav_group_send_audio(tox, group_number, pcm, samples, channels,
                                   sample_rate);
  if (ret == -1) {
...
static PyMethodDef ToxAVCore_methods[] = {
    ...
    {"call_control", (PyCFunction)ToxAVCore_call_control, METH_VARARGS,
     ...
     "Returns True on success.\n\n"},
    {"audio_set_bit_rate", (PyCFunction)ToxAVCore_audio_set_bit_rate,
     METH_VARARGS,
     "audio_set_bit_rate(friend_number, audio_bit_rate)\n"
     "Set the bit rate to be used in subsequent audio frames."
     "Returns True on success.\n\n"},
    {"video_set_bit_rate", (PyCFunction)ToxAVCore_video_set_bit_rate,
     METH_VARARGS,
    ...
    {"audio_send_frame", (PyCFunction)ToxAVCore_audio_send_frame, METH_VARARGS,
     "audio_send_frame(friend_number, pcm, sample_count, channels, "
     "sampling_rate)\n"
