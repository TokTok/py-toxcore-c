...
static void ToxAVCore_callback_call(ToxAV *toxAV, uint32_t friend_number, bool audio_enabled,
                                    bool video_enabled, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_call", "iii", friend_number, audio_enabled,
                      video_enabled);
}

static void ToxAVCore_callback_call_state(ToxAV *toxAV, uint32_t friend_number, uint32_t state,
                                          void *self) {
  PyObject_CallMethod((PyObject *)self, "on_call_state", "ii", friend_number, state);
}

static void ToxAVCore_callback_audio_bit_rate(ToxAV *toxAV, uint32_t friend_number,
                                              uint32_t audio_bit_rate, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_audio_bit_rate", "ii", friend_number, audio_bit_rate);
}

static void ToxAVCore_callback_video_bit_rate(ToxAV *toxAV, uint32_t friend_number,
                                              uint32_t video_bit_rate, void *self) {
  PyObject_CallMethod((PyObject *)self, "on_video_bit_rate", "ii", friend_number, video_bit_rate);
}

static void ToxAVCore_callback_audio_receive_frame(ToxAV *toxAV, uint32_t friend_number,
  ...
  PyObject_CallMethod((PyObject *)self, "on_audio_receive_frame", "i" BUF_TCS "iii", friend_number,
                      (const char *)pcm, length, sample_count, channels, sampling_rate);
...
static void i420_to_rgb(int width, int height, const uint8_t *y, const uint8_t *u, const uint8_t *v,
...
static void ToxAVCore_callback_video_receive_frame(ToxAV *toxAV, uint32_t friend_number,
  ...
  PyObject_CallMethod((PyObject *)self, "on_video_receive_frame", "iii" BUF_TCS, friend_number,
 ...
 */
static void ToxAVCore_callback_add_av_groupchat(/*Tox*/ void *tox, uint32_t groupnumber,
...
static void ToxAVCore_callback_join_av_groupchat(/*Tox*/ void *tox, uint32_t groupnumber,
                                                 uint32_t peernumber, const int16_t *pcm,
                                                 unsigned int samples, uint8_t channels,
                                                 unsigned int sample_rate, void *self) {
  PyGILState_STATE gstate = PyGILState_Ensure();
...
static PyObject *ToxAVCore_audio_send_frame(ToxAVCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "i" BUF_TCS "iii", &friend_number, (uint8_t **)&pcm, &pcm_length,
...
static PyObject *ToxAVCore_group_send_audio(ToxAVCore *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "i" BUF_TCS "iii", &group_number, &pcm, &length, &samples, &channels,
                        &sample_rate)) {
    return NULL;
  }
